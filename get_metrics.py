...
AWS_ACCESS_KEY_ID="ASIAWLDJOTJ7Y7P7Y7ZK"
AWS_SECRET_ACCESS_KEY="0p6f5xUoMGXf7IY5GFWZblmL0TVSBonoybwvTMUd"
AWS_SESSION_TOKEN=""
...
def get_metrics():
    ...
    for evald in new_trainer.state.eval_metrics.keys():
        with open('resnet50_medium_metrics.txt', 'w+') as f:
    ...
    for evald in new_trainer.state.eval_metrics.keys():
        for name, metric in new_trainer.state.eval_metrics[evald].items():
            if name == 'PerClassAccuracy':
                l = metric.compute().cpu().numpy()
                # ls = np.array2string(l, separator=', ')
                for c,a in reversed(sorted(zip(l, classes))[-100:]):
                    print(f"{c}: {a}")
